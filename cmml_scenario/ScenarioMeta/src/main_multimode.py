...
def get_model(useritem_embeds, user_neighbor_dict, item_neighbor_dict, criterion, config):
    ...
    if 'support' in config:
        ...
        trainable_num_model = sum(p.numel() for p in model.parameters() if p.requires_grad)
        trainable_num_gradient_model = sum(p.numel() for p in gradient_model.parameters() if p.requires_grad)
        print("trainable_num_model",trainable_num_model,'trainable_num_gradient_model',trainable_num_gradient_model)
