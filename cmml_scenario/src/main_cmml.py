...
from modules import EmbedRecommender, InteractionRecommender, HybridRecommender, Meta_final_cross_InteractionRecommender, Meta_mlp_cross_InteractionRecommender, Meta_softmodule_cross_InteractionRecommender
import random
import os 
...
def get_model(useritem_embeds, user_neighbor_dict, item_neighbor_dict, criterion, config):
    ...
    if model_type == 'mapping':
        ...
        bias = config['bias_init']
        lstm_bias = config['lstm_bias']
        bias_value = config['bias_init_value']
        model = Meta_mlp_cross_InteractionRecommender(useritem_embeds, way=hybrid, input_shape=input_shape, hyper_hidden=hyper_hidden, lstm_hidden=lstm_hidden, bias_init=bias, lstm_bias=lstm_bias, bias_value = bias_value)
    elif model_type == 'meta_final':
        bias = config['bias_init']
        lstm_bias = config['lstm_bias']
        bias_value = config['bias_init_value']
        model = Meta_final_cross_InteractionRecommender(useritem_embeds, way=hybrid, input_shape=input_shape, hyper_hidden=hyper_hidden, lstm_hidden=lstm_hidden, bias_init=bias, lstm_bias=lstm_bias, bias_value=bias_value)
    elif model_type == 'meta_soft':
        model = Meta_softmodule_cross_InteractionRecommender(useritem_embeds, way=hybrid, input_shape=input_shape, hyper_hidden=hyper_hidden, lstm_hidden=lstm_hidden)
    elif model_type == 'meta_mlp_sigmoid':
        bias = config['bias_init']
        lstm_bias = config['lstm_bias']
        bias_value = config['bias_init_value']
        model = Meta_mlp_cross_InteractionRecommender(useritem_embeds, way=hybrid, condition_way='sigmoid', input_shape=input_shape, hyper_hidden=hyper_hidden, lstm_hidden=lstm_hidden, bias_init=bias, bias_value = bias_value)
    else:
