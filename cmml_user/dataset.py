...
class pretrain_dataset(Dataset):
    ...
    def __getitem__(self, item):
        user_id = int(self.data[item][0])
        item_id = int(self.data[item][1])
        label = self.data[item][2]
        
        user_feature, item_feature = self.user_dict[user_id], self.movie_dict[item_id]
        feature = torch.cat([item_feature, user_feature], dim=1).reshape(-1)
...
class Metamovie_new(Dataset):
    def __init__(self, args, partition='train', test_way=None, path=None):
        super(Metamovie_new, self).__init__()
        #self.dataset_path = args.data_root
        self.partition = partition
        #self.pretrain = pretrain
        ...
        for _, user_id in tqdm(enumerate(list(self.dataset_split.keys()))):
            ...
            #else:
            #    self.final_index.append(user_id)
            if len(self.dataset_split[str(u_id)]) > 100:
                self.dataset_split[str(u_id)] = self.dataset_split[str(u_id)][:100]
            self.final_index.append(user_id)
